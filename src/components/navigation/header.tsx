...
import { useBackend, useWindow, useScrollPosition } from "@/hooks";
...
// Move DesktopHeader and MobileHeader out of Header
// 1. Define DesktopHeader and MobileHeader as separate components above Header
// 2. Pass all required props from Header
// 3. In Header, render <DesktopHeader ... /> or <MobileHeader ... />
...
const Header = () => {
  ...
  const { resolution, windowWidth } = useWindow();
  const { scrollDirection, scrollPos } = useScrollPosition();
  ...
  useEffect(() => {
    ...
    console.log("Pathname use effect");
  }, [pathname, backBtn]);
  ...
  useEffect(() => {
    ...
    console.log("BackBtn use effect");
    return () => {
  ...
  useEffect(() => {
    console.log("HomeActions use effect");
  ...
  useEffect(() => {
    console.log("ScrollDirection use effect");
    if (pathname === "/") {
  ...
  useEffect(() => {
    console.log("ScrollPos use effect");
  ...
  useEffect(() => {
    console.log("UserNotifications use effect");
  ...
  return windowWidth >= resolution.lg ? (
    <DesktopHeader
      pathname={pathname}
      isOnTopScrollOnHome={isOnTopScrollOnHome}
      date_filter={date_filter}
      setParams={setParams}
      params={params}
      queryForEvents={queryForEvents}
      search_results={search_results}
      user={user}
      has_new_notifs={has_new_notifs}
    ...
    <MobileHeader
      ...
      has_new_notifs={has_new_notifs}
      toggleNotificationsModal={toggleNotificationsModal}
      mobileButtonHandler={mobileButtonHandler}
